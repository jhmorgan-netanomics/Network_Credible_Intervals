...
\paragraph{Methodological note: Kendall $\tau_b$ as the missingness--centrality correlation.} An early implementation of the sampler used Pearson correlation between the missingness indicator and centrality, mirroring \citet{SmithMorganMoody2022}'s formulation. This was discovered to produce dramatically asymmetric achievable-$\rho$ ceilings on heavy-tailed networks (Marvel: positive ceiling $+0.4$ vs.~negative ceiling $-0.03$), making the negative-$\rho$ direction effectively inaccessible on networks with long low-degree tails. The sampler was refactored to use Kendall's $\tau_b$ with rank-based sampling weights, and the production corpus regenerated. The Kendall formulation produces symmetric ceilings across the full corpus and aligns the metric with the user's rank-order intuition about missingness. The two ceilings that constrain achievable $\rho$ under Kendall (a rate-bounded absolute ceiling $\sqrt{2 p (1-p)}$ and a network-specific practical ceiling) are both documented in the Joint Sampler subsection below. This change is reflected throughout the validation: the Phase~1 sampler reports realized $\rho$ as Kendall $\tau_b$, the Phase~2 framework solves its $\beta$ against the analytic Kendall formula, and downstream coverage analysis conditions on Kendall-metric realized values rather than Pearson-metric ones.
...
Missingness is induced at the node level. The probability that node~$i$ is selected for removal is a convex combination of a rank-based centrality signal and uniform noise:
\[
\text{prob}_i = b \cdot r_i + (1 - b) \cdot u_i,
\]
where $r_i = (\text{tiedrank}(c_i) - 1) / (N - 1)$ is the centrality rank scaled to $[0, 1]$, $u_i \sim \text{Uniform}(0, 1)$, and the mixing scalar~$b \in [0, 1]$ controls selectivity. The centrality driver $c_i$ is the binarized \emph{in-degree} for directed networks and the binarized \emph{degree} for undirected networks --- following \citet{SmithMorganMoody2022} for the directed case and extending it naturally to the undirected case. Negative target~$\rho$ is handled by sign-flipping the probability vector ($\text{prob}_i \leftarrow 1 - \text{prob}_i$) before sampling, which preferentially selects low-rank nodes while keeping all weights in $[0, 1]$.

\paragraph{Rank-based weighting under Kendall $\tau_b$.} The sampler weights node selection by centrality \emph{rank} rather than raw value. This is a substantive methodological choice that distinguishes the framework from a Pearson-based formulation. The user's intuition about missingness --- ``low-ranked nodes are likely to be missing'' --- is fundamentally a rank-order statement, not a linear-correlation statement. Min--max normalization of raw centrality values, the previously adopted scheme, produces severely asymmetric sampling behavior on heavy-tailed networks: a few hubs dominate the normalized vector while the many low-degree nodes are nearly indistinguishable. Rank-based normalization gives every position in the centrality distribution equal weight-density, which restores symmetry across the sign of $\rho$.

\paragraph{The bisection target: realized Kendall $\tau_b$.} The framework departs from \citet{SmithMorganMoody2022} in what quantity~$\rho$ controls. The prior work defined~$\rho$ as the Pearson correlation between centrality and the \emph{probability} of being a non-respondent --- a deterministic, sampler-recipe quantity that bisection on~$b$ can target directly. The framework instead defines~$\rho$ as the expected realized \emph{Kendall} $\tau_b$ between the dropped-status indicator and centrality:
\[
\rho_{\text{target}} = \mathbb{E}\!\left[\tau_b\!\left(\mathbb{1}_{\text{dropped}}, c\right)\right],
\]
where the expectation is over the weighted-without-replacement draw given~$b$, target rate, and the probability vector above. Kendall $\tau_b$ is itself a rank-based correlation, so changing from raw to ranked centrality on the weighting side coheres with measuring outcomes by rank concordance: both the input weighting and the output metric live in rank space. Kendall $\tau_b$ also handles ties on the centrality side (frequent in unweighted networks where many nodes share the same degree) without the ad-hoc tie-breaking that Pearson on tied data requires.

\paragraph{Monte Carlo bisection.} The realized Kendall target is not analytically invertible in~$b$, so the bisection is run against a Monte Carlo estimate. At each candidate~$b$ the sampler draws $M = 50$ inner weighted-without-replacement samples at the target rate, computes the empirical mean of $\tau_b(\mathbb{1}_{\text{dropped}}, c)$ across them using \texttt{StatsBase.corkendall} (Knight's algorithm, $O(N \log N)$ per call), and bisects that mean against~$|\rho_{\text{target}}|$ (the sign of~$\rho$ is handled separately via the probability-vector flip). The bisection tolerance is set to~$0.02$ to accommodate the finite-sample noise of the M=50 inner estimate (standard error roughly $0.015$ on networks at typical centrality variance). Each bisection iteration uses deterministic per-iteration seeds derived from the master seed and iteration index, so the procedure is reproducible from a single master seed regardless of thread scheduling. Per-replicate timing on Marvel ($N = 6{,}486$) is approximately 0.1 seconds under Kendall vs. roughly 0.04 seconds under Pearson; the cost increase is acceptable given the methodological gain.

\paragraph{Two distinct ceilings on the achievable $\rho$.} Under Kendall $\tau_b$ on a binary indicator (missingness, with proportion $p = k/N$) and a continuous variable (centrality), realized correlation is structurally bounded by \emph{two} distinct ceilings:

\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item \textbf{Rate-bounded absolute ceiling.} The mathematical maximum is
  \[
  |\tau_b|_{\max} = \sqrt{\frac{2 \cdot k \cdot (N - k)}{N \cdot (N - 1)}} \approx \sqrt{2 \cdot p \cdot (1 - p)}.
  \]
  This depends only on the missingness rate, not on the network. At $p = 0.10$, $|\tau_b|_{\max} \approx 0.42$; at $p = 0.50$, $|\tau_b|_{\max} \approx 0.71$. No sampling configuration on any network can produce realized $\tau_b$ exceeding this absolute ceiling.

  \item \textbf{Practical tie-bounded ceiling.} On networks with centrality ties (most unweighted networks), the achievable $|\tau_b|$ is reduced below the absolute ceiling because tied centralities contribute to the denominator of $\tau_b$. The practical ceiling is network-specific and is what the sampler can actually reach.
\end{itemize}

The sampler detects ceiling-hits by probing $b = 1$ at the start of every bisection: if the resulting Monte Carlo estimate of the indicator correlation falls below the target by more than the tolerance, the sampler returns the ceiling estimate, sets \texttt{bisection\_status~=~:ceiling\_hit}, and records the achieved realized~$\rho$ as the actual condition rather than reporting the unattained nominal target.

\paragraph{Symmetric ceilings under Kendall $\tau_b$.} Under the rank-based weighting and Kendall metric, the achievable ceilings are \emph{symmetric} between positive and negative directions on every corpus network. The production corpus (78{,}000 rows under Kendall) confirms this empirically: Marvel at nominal $\rho = \pm 0.75$ produces realized values of $-0.236$ and $+0.230$ respectively. The corresponding values on Scotland are $-0.232$ and $+0.235$; on Moreno, $-0.250$ and $+0.239$. Networks ranging from $N = 70$ (Moreno) to $N = 6{,}486$ (Marvel), from light-tailed to heavy-tailed, from directed to undirected, all produce realized $|\rho|$ within a narrow band at fixed nominal target.

This is a substantive improvement over the prior Pearson-based formulation, which produced dramatically asymmetric ceilings on heavy-tailed networks: under Pearson, Marvel's positive ceiling was approximately $+0.4$ but its negative ceiling was approximately $-0.03$, making it impossible for users with peripheral-missingness priors to express their beliefs meaningfully on heavy-tailed networks. The Kendall switch eliminates this asymmetry. The framework now records realized values that scale uniformly across the corpus, and downstream coverage analysis can condition on realized $\rho$ without per-network asymmetry corrections.

\paragraph{Saturation handling at extreme~$b$.} When the probability vector has fewer than~$k = \lfloor \text{rate} \cdot N \rfloor$ strictly positive entries --- the regime where $b = 1$ on a centrality-saturated fixture concentrates all weight on one or two nodes --- standard weighted-without-replacement sampling cannot proceed: there are fewer positive weights than draws required. The sampler falls back to a deterministic-positive-plus-uniform-fill draw: all positive-weight nodes are selected, and the remaining $k - n_+$ slots are filled by uniform draws from the zero-weight nodes. This produces a well-defined dropped set whose realized indicator correlation is below any target $\rho$ near the ceiling, consistent with the bisection's \texttt{:ceiling\_hit} classification. The saturation path was relevant under Pearson when raw min--max normalization concentrated weight on hubs; under Kendall with rank-based normalization the probability vector is more uniformly distributed and saturation is rarer.

\paragraph{Recording nominal, realized, and degeneracy.} The sampler always records three things for each degraded network: the \emph{nominal} (rate,~$\rho$) requested, the \emph{realized} (rate,~$\rho$) actually achieved by the sampled dropped-node set (reported as Kendall $\tau_b$), and a \emph{degeneracy flag}. Realized values differ from nominal even when the bisection succeeds in expectation, because each finite draw has sampling noise --- severe at small~$N$ and low rates (5\% of 70 nodes is three to four nodes, a very noisy realized correlation). Recording both lets coverage be tabulated against either the nominal grid or the realized conditions, and it feeds the per-cell degeneracy rate used for grid trimming below.
...
Before running the sampler against the corpus, a nine-test harness verifies that it does what its API claims. The harness follows the convention of the Phase~0 test suites (e.g., \texttt{run\_synthetic\_path\_centrality\_tests}): a single driver function that runs all checks, prints a one-line status per check, and returns a Boolean for use in regression. Each check is callable in isolation for debugging.

\paragraph{Test 1: Bisection convergence on a tractable network.} On Scotland (undirected, $N=108$, moderate skew), target $\rho = +0.10$ at rate~$0.10$. Draw 100 replicates. Assert that the mean realized~$\rho$ across the 100 draws matches target within tolerance~$0.03$. The tolerance is loosened from a deterministic-bisection ideal because two layers of Monte Carlo noise compose: the M=50 inner-sample bisection estimate has standard error roughly~$0.015$, and the 100-replicate harness mean adds another roughly~$0.010$, giving a triangle-inequality bound of about~$0.03$. The target $\rho = +0.10$ is chosen to sit comfortably inside Scotland's practical Kendall $\tau_b$ ceiling at rate~$0.10$ (approximately~$0.20$ on this fixture); a target above the practical ceiling exercises the ceiling-hit path tested separately in Test~1b. Single-replicate realized~$\rho$ is noisy; the test is on the mean across many seeds, not on any single draw.

\paragraph{Test 1b: Rate-bounded ceiling detection.} On Scotland (undirected, $N=108$), target $\rho = +0.25$ at rate~$0.10$. Draw 20 replicates. The target sits above Scotland's practical Kendall $\tau_b$ ceiling at this rate but well below the rate-bounded absolute ceiling $\sqrt{2 \cdot p \cdot (1 - p)} \approx 0.42$. The bisection is expected to detect that the target is unreachable: the $b = 1$ probe Monte Carlo estimate falls below~$0.25$, and the sampler returns \texttt{bisection\_status~=~:ceiling\_hit}. The test asserts (a) 20/20 replicates report \texttt{:ceiling\_hit}, and (b) no replicate's realized $|\rho|$ exceeds the theoretical absolute ceiling $\sqrt{2 \cdot p \cdot (1 - p)}$ plus a tolerance of~$0.05$ for finite-sample noise. This is a new test under the Kendall refactor: it documents the rate-bounded absolute ceiling as a structural property and verifies the sampler honors it.
...
The harness above verifies sampler correctness on small networks where the per-test wall-clock cost is negligible. Marvel ($N = 6{,}486$, $E = 77{,}227$) raises questions the small-network harness cannot answer: per-replicate timing at corpus-large scale, bisection convergence on a centrality vector with realistic variance, degeneracy-detector behavior on a large clustered network, and the budget profile for a full validation grid. A separate diagnostic, \texttt{run\_marvel\_stress\_test}, was re-run after the Kendall refactor to characterize this behavior.

\paragraph{Per-replicate timing.} The centrality computation (one-time per network, cached for all replicates) completes in~$0.039$~seconds on Marvel, with mean centrality~$23.8$ matching the expected $2E/N = 23.81$ to four significant figures. Per-replicate wall-clock at the $(\rho = +0.25, \text{rate} = 0.10)$ design cell averages~$0.102$~seconds across 20 draws under the Kendall metric (Knight's algorithm via \texttt{StatsBase.corkendall}), with range~$0.093$--$0.123$~seconds. This is dominated by the Monte Carlo bisection's~$\sim$1500 inner sampler operations at Marvel scale; the materialization and degeneracy check are sub-millisecond. The Kendall metric is roughly 2.5x slower per inner sample than the previous Pearson formulation but still well within the budget.

\paragraph{Ceiling detection on Marvel.} All 20 replicates report \texttt{bisection\_status~=~:ceiling\_hit} at the $(\rho = +0.25, \text{rate} = 0.10)$ design cell, with zero \texttt{:failed\_other} statuses. This is expected and matches Test~1b's prediction: Marvel's practical Kendall $\tau_b$ ceiling at rate~$0.10$ is approximately~$0.16$, and the target $\rho = +0.25$ exceeds this practical ceiling. The sampler correctly identifies the unreachable target and records the achieved ceiling value rather than reporting the unattained nominal target. Under the prior Pearson formulation, Marvel showed asymmetric ceilings (positive $\rho_{\max} \approx +0.4$, negative $\rho_{\min} \approx -0.03$); under Kendall the ceiling is symmetric, with mean realized $|\rho|$ at the ceiling approximately $0.16$ in both directions.
...
\paragraph{Production corpus confirms symmetric behavior.} The production corpus regenerated under Kendall (78{,}000 rows, eighteen networks counting unweighted variants, wall-clock 11~minutes 17~seconds on 8 threads) confirms the symmetric-ceiling property across the full network set. On Marvel at nominal $\rho = \pm 0.75$, realized values are $-0.236$ and $+0.230$. On Moreno, $-0.250$ and $+0.239$. On Scotland, $-0.232$ and $+0.235$. The symmetric magnitudes hold uniformly across networks ranging from $N = 70$ to $N = 6{,}486$, light-tailed to heavy-tailed, directed to undirected. The corpus shows 54{,}598 of 78{,}000 rows ceiling-hit (70\%), reflecting that the original grid was designed under the prior Pearson scheme where the achievable ceiling was network-dependent and often higher at low rates; under Kendall many grid cells at low rates fall above the practical ceiling, and the framework surfaces this structural infeasibility honestly via \texttt{:ceiling\_hit} rather than silently delivering unattained targets.

\paragraph{Budget profile.} At mean~$0.102$~seconds per replicate and a single-network grid of $5 \times 6 \times 100 = 3000$ cells, the per-network wall-clock estimate for Marvel is approximately~$5$~minutes for one mechanism. Scaling to the sixteen-network corpus gives a single-mechanism corpus budget of approximately~$1.5$~hours; the realized production-corpus run completed both mechanisms on directed networks in approximately~$11$~minutes on 8~threads (see the realized timing above). These budgets fit comfortably within an overnight run on a single workstation.
