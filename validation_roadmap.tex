...
\paragraph{Asymmetric ceilings on heavy-tailed networks.} The achievable-$\rho$ ceiling is, in general, \emph{not symmetric} between positive and negative targets. The asymmetry depends on how mass is distributed across the centrality vector. On networks where centrality is concentrated in a small number of hubs with a long thin tail (Marvel Universe is the corpus exemplar: a one-mode projection of a bipartite hero-comic network thresholded at co-appearance count~$\geq 2$, which produces a centrality vector with a few dozen high-degree hubs and several thousand nodes at degree~1 or~2), the positive-$\rho$ direction can achieve moderate realized correlations because the hubs are distinguishable from one another and from the low-degree mass: at $b = 1$, the prob vector concentrates on those hubs and the realized indicator correlation reaches roughly $+0.4$. The negative-$\rho$ direction, by contrast, asks the sampler to preferentially drop the lowest-centrality nodes --- but there are several thousand of them, all at degree~1 or~2, all indistinguishable from one another in centrality terms. Sign-flipping the prob vector at $b = 1$ produces probabilities of roughly $1 - \tilde{c}_i$ for each non-hub, which differs across the low-degree mass by amounts that are dominated by sampling noise. The realized cor between the dropped set and centrality consequently stays close to zero (we observe realized~$\rho$ around $-0.04$ on Marvel at nominal $\rho = -0.75$ across all rates). This is a property of the prepared network, not a sampler limitation: the threshold-based projection deliberately collapses the periphery into a low-degree mass to clean up the co-appearance signal. The framework records the realized value honestly via \texttt{:ceiling\_hit} and the downstream coverage analysis conditions on realized rather than nominal $\rho$, so asymmetric ceilings are absorbed correctly. The corpus-level reporting includes per-network ceiling profiles in both directions so the reader can see which networks support strong negative-$\rho$ inference and which do not.
