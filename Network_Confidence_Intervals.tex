...
\paragraph{Feasibility check.} The framework provides a public function \texttt{feasible\_rho\_range} that probes the sampling mechanism at extreme configurations to estimate the practical $[\rho_{\min}, \rho_{\max}]$ achievable on a given network at a given rate. The wrapper \texttt{reconstruct\_network} calls this function at startup and emits a warning if the user's $\rho$ falls outside the practical range, with three tiers of warning intensity (silent, note, prompt) depending on the degree of infeasibility.

\paragraph{What happens when $\rho$ exceeds the practical ceiling.} The framework does not silently clip or rescale. Instead, the Phase~1 sampler's bisection on the mixing scalar~$b$ saturates at the extreme value, and the resulting \emph{realized} $\rho$ is whatever the network can structurally produce. Phase~2's bin-distribution solver then honors the realized $\rho$ rather than the user's nominal request. Diagnostic output reports both nominal and realized values, allowing the user to interpret credible intervals appropriately.
...
Find a distribution~$q$ over degree bins $\{1, \ldots, K\}$ such that drawing $\Nadd$ bins from~$q$ yields realized Kendall $\tau_b$ correlation~$\rho$ between the bin index and the missingness indicator across all $N + \Nadd$ nodes. Use the logistic form
\[
q_b \propto \exp(\beta \cdot \tilde{b}),
\]
where $\tilde{b}$ is the standardized bin index. Solve numerically for~$\beta$ such that the induced Kendall $\tau_b$ matches~$\rho$:
\begin{itemize}[leftmargin=*, itemsep=2pt]
...
\paragraph{Analytic Kendall $\tau_b$ formula.} Because bins are ordinal and missingness is binary, Kendall $\tau_b$ has a closed-form expression as a function of $\beta$, $K$, $N$, and $\Nadd$. Only cross-pairs (one observed node, one added node) contribute to concordant/discordant counts; same-status pairs are tied on the indicator. With observed nodes uniformly distributed across bins ($N/K$ per bin) and added nodes distributed by $q$:
\[
C = \frac{N}{K} \cdot \Nadd \cdot \sum_{k=1}^{K} q_k \cdot (k - 1), \qquad
D = \frac{N}{K} \cdot \Nadd \cdot \sum_{k=1}^{K} q_k \cdot (K - k),
\]
\[
T_x = \frac{N \cdot \Nadd}{K}, \qquad
T_y = \frac{N^2 (K - 1)}{2K} + \frac{\Nadd^2}{2}\left(1 - \sum_k q_k^2\right),
\]
\[
\tau_b = \frac{C - D}{\sqrt{(C + D + T_x)(C + D + T_y)}}.
\]
This is the function being numerically inverted to solve for~$\beta$. The formula is monotone increasing in~$\beta$, so a straightforward bisection on $\beta \in [-50, 50]$ converges in a small number of iterations to tight tolerance.

\paragraph{$K$-dependent ceiling.} The maximum achievable $|\tau_b|$ from Step~5's mechanism depends on~$K$: larger~$K$ reduces the bin-side ties $T_x$ and lifts the ceiling closer to the rate-bounded absolute $\sqrt{2 p (1 - p)}$. Smaller~$K$ increases bin ties and lowers the ceiling. This is one reason the framework's default $K$ is chosen by \texttt{find\_optimal\_K} (Section~\ref{sec:three-priors}): for a user-specified $\rho$ near the rate-bounded ceiling, only sufficiently large $K$ can avoid \texttt{:ceiling\_hit} in the bisection.

\paragraph{$\beta$-solver status.} The solver returns one of three statuses: \texttt{:converged} (bisection found $\beta$ within tolerance of the target), \texttt{:ceiling\_hit} (target exceeds the $K$-dependent achievable maximum; $\beta$ saturates at the bracket endpoint), or \texttt{:failed\_other} (degenerate case, e.g., $\Nadd = 0$ with nonzero target). The status is recorded in diagnostics.
...
\subsection{The Three-Prior Algorithmic Contract}
\label{sec:three-priors}

The framework operates as an algorithmic contract that honors three priors on each replicate:

\begin{enumerate}[leftmargin=*, itemsep=4pt]
  \item \textbf{Prior~1 (proportion missing).} The user supplies $\pinode$. The framework honors this via the arithmetic identity $\Nadd / (N + \Nadd) = \pinode$ (modulo integer rounding). Verified by the inputs to \texttt{\_determine\_n\_add}.

  \item \textbf{Prior~2 (centrality-missingness correlation).} The user supplies $\rho$. The framework honors this via the $\beta$ solver in Step~5, which finds $\beta$ such that the analytic Kendall $\tau_b$ between bin index and missingness equals $\rho$ within tolerance. When $\rho$ exceeds the $K$-dependent ceiling, the solver returns \texttt{:ceiling\_hit}, and the realized $\rho$ is the maximum achievable at the given $K$, $N$, and $\Nadd$. Both \texttt{:converged} and \texttt{:ceiling\_hit} are contract-compliant responses; the framework's job is to tell the user honestly what it could and could not achieve.

  \item \textbf{Prior~3 (E/I distribution conditional on degree).} This prior is \emph{inferred} from $\Gobs$ rather than user-supplied. The framework computes the empirical conditional distribution $P(\text{E/I bin} \mid \text{degree bin})$ from observed nodes and uses it to assign E/I bins to added nodes (with uniform fallback for empty degree bins). The contract is that added nodes' E/I distribution matches $\Gobs$'s empirical conditional row-by-row.
\end{enumerate}

\paragraph{What the contract guarantees and what it does not.} The contract guarantees the framework will honor each prior \emph{algorithmically}: the realized values across replicates will match the priors within tolerance, or the framework will report exactly where it could not (via \texttt{:ceiling\_hit} status flags). The contract does \emph{not} guarantee that the user's priors are \emph{correct}: a user who specifies $\rho = 0$ when the truth is $\rho = 0.5$ will receive miscalibrated credible intervals, but the framework will honor $\rho = 0$ as requested.

\paragraph{Automatic $K$ selection.} The function \texttt{find\_optimal\_K} searches for the largest $K$ that satisfies all three priors. Because the Phase~2 analytic Kendall ceiling depends on $K$ (larger $K$ permits larger realized $\rho$), users specifying $\rho$ near the rate-bounded ceiling benefit from larger $K$ to avoid \texttt{:ceiling\_hit}. The search is preceded by a feasibility check via \texttt{feasible\_rho\_range}: if $\rho$ is structurally infeasible, the function returns immediately with \texttt{:rho\_infeasible} rather than searching $K$ values that cannot succeed.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  ...
  \begin{itemize}[leftmargin=*, itemsep=1pt]
    \item Nominal vs. realized $\rho$ (Kendall $\tau_b$) per replicate, with summary across replicates
    \item Nominal vs. realized missingness rate per replicate
    \item $\beta$ solver status distribution (\texttt{:converged}, \texttt{:ceiling\_hit}, \texttt{:failed\_other}) across replicates
    \item Feasibility output from \texttt{feasible\_rho\_range}: practical $[\rho_{\min}, \rho_{\max}]$ at the user's missingness rate, MC standard errors, asymmetry flag
    \item $K$ selected by \texttt{find\_optimal\_K} (if \texttt{K = :auto}) and per-$K$ test trace
    \item Transformation parameters used (if weight semantics required transformation)
    \item Community structure used and E/I distributions
    \item Binning mode (2D or fallback to 1D) and any fallback reason
    \item Per-replicate degeneracy flags (giant-component fraction, empty bins, etc.) for downstream filtering
  \end{itemize}
\end{itemize}
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  ...
  \item Silent enforcement of structurally infeasible $\rho$ priors. When the user's $\rho$ exceeds the rate-bounded or tie-bounded ceiling, the framework operates at the achievable ceiling, reports the realized $\rho$ alongside the nominal, and emits a diagnostic. It does not silently rescale or clip the user's input.
\end{itemize}
