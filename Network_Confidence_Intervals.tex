...
\item A user-specified non-uniform edge-weight missingness mechanism. Stage~2 distributes the weight budget across existing and imputed edges in proportion to current weight times an endpoint-survival tilt drawn from the same $\rho$-field that governs node missingness; it reduces to current-weight-proportional allocation (the Bellutta MCAR corner) only at $\rho = 0$. There is no separate dial for edge-weight non-uniformity.
